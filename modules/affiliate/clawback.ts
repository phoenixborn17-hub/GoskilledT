...
export function buildClawbackTxns(orderId: string, commissionTxns: TxSpec[]): TxSpec[] {
  return commissionTxns.map((txn) => {
    const wallet = txn.legs.find((l) => l.account.kind === "USER_WALLET");
    if (!wallet || wallet.account.kind !== "USER_WALLET") throw new Error(`No wallet leg in ${txn.idempotencyKey}`);
    const level = Number(txn.idempotencyKey.split(":").pop()) as Level;
    return {
      type: "CLAWBACK" as const,
      idempotencyKey: clawbackIdempotencyKey(orderId, wallet.account.userId, level),
      refType: "Order",
      refId: orderId,
