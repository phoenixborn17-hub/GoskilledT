...
export type WebhookEventKind =
  | { kind: "payment.captured"; orderId: string; paymentId: string; amountInPaise: number }
  | { kind: "payment.failed"; orderId: string; paymentId: string }
  | { kind: "refund.processed"; orderId: string; refundId: string; amountInPaise: number };
...
export interface Decision { actions: Action[]; note?: string }
...
export function decideWebhookActions(
  ...
  if (opts.alreadyProcessed) return { actions: [], note: "duplicate webhook — idempotent skip" };
  if (!order) return { actions: [], note: "unknown order — log only" };
  const now = opts.now ?? new Date();

  switch (event.kind) {
    case "payment.captured": {
      if (order.status === "PAID") return { actions: [], note: "already paid — idempotent skip" };
      if (order.status === "REFUNDED") return { actions: [{ do: "FLAG_MANUAL_REVIEW", orderId: order.id, reason: "capture after refund" }] };
      if (event.amountInPaise !== order.amountInPaise)
        return { actions: [{ do: "FLAG_MANUAL_REVIEW", orderId: order.id, reason: `amount mismatch: got ${event.amountInPaise}, expected ${order.amountInPaise}` }] };
    ...
    case "payment.failed": {
      if (order.status !== "CREATED") return { actions: [], note: "failure for non-pending order — skip" };
      return { actions: [{ do: "MARK_FAILED", orderId: order.id }] };
    }
    case "refund.processed": {
      if (order.status !== "PAID") return { actions: [{ do: "FLAG_MANUAL_REVIEW", orderId: order.id, reason: "refund for non-paid order" }] };
      if (!order.paidAt) return { actions: [{ do: "FLAG_MANUAL_REVIEW", orderId: order.id, reason: "refund but paidAt missing" }] };
      ...
      return { actions: [
        { do: "MARK_REFUNDED", orderId: order.id },
        { do: "REVOKE_ENROLLMENTS", orderId: order.id },
        { do: "FLAG_MANUAL_REVIEW", orderId: order.id, reason: "post-window refund — manual future-earnings adjustment (DR-025)" },
      ]};
    }
  }
}
