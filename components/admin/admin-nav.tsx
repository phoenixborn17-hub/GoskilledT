...
export function AdminNav() {
  ...
  return (
    <>
      ...
      <nav
        aria-label="Admin"
        className="fixed inset-y-0 left-0 z-20 hidden w-56 flex-col border-r border-charcoal/10 bg-charcoal p-4 md:flex"
      >
        <span className="mb-6 px-2 font-heading text-lg font-bold text-white">
          GoSkilled <span className="text-white/50">Admin</span>
        </span>
      ...
      <nav
        aria-label="Admin"
        className="sticky top-0 z-20 flex gap-1 overflow-x-auto border-b border-charcoal/10 bg-charcoal px-2 py-2 md:hidden"
      >
        {TABS.map(({ href, label, Icon, exact }) => {
          const active = isActive(pathname, href, exact);
          return (
