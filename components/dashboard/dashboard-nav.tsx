...
const TABS = [
  ...
  },
  {
    ...
    label: "Earn",
    Icon: Coins,
    match: (p: string) => p.startsWith("/dashboard/earn"),
  },
  {
  ...
  return (
    <>
      ...
      <nav
        ...
        className="fixed inset-y-0 left-0 z-20 hidden w-56 flex-col border-r border-charcoal/10 bg-white p-4 md:flex"
      >
        <span className="mb-6 px-2 font-heading text-lg font-bold text-brand">
          GoSkilled
        </span>
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    active
                      ...
                      : "text-charcoal/70 hover:bg-brand/5",
                  )}
                >
      ...
      <nav
        aria-label="Dashboard"
        className="glass fixed inset-x-0 bottom-0 z-20 grid grid-cols-4 border-t border-charcoal/10 md:hidden"
      >
        {TABS.map(({ href, label, Icon, match }) => {
          const active = match(pathname);
          return (
