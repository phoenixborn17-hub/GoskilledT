...
| 10 | Welcome / Lesson-0 video | onboarding/dashboard | Content | PENDING | Founder | Slot editable via `/admin/catalog` (lesson `videoAssetId`) — set the Stream UID when the intro is recorded | 60–90s recorded intro | LAUNCH (if Day-0 spec approved) | Pre-launch | Video asset id now entered through catalog CRUD (GPS-M4). |
