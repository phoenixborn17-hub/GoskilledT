...
import { formatDuration, courseStats, packagesIncludingCourse, priceLabel } from "../../../lib/catalog/shape";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return pageMetadata({
    title: course.title,
    description: course.summary ?? `Learn ${course.title} with GoSkilled — practical, job-ready skills.`,
...
export default async function CourseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const [course, packages] = await Promise.all([getCourseDetail(slug), listPackages()]);
  if (!course) notFound();
    ...
    <>
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-10">
        ...
        <section aria-labelledby="outcomes" className="reveal mt-8">
          <h2 id="outcomes" className="font-heading text-xl font-bold">What you&apos;ll learn</h2>
          <ul className="mt-3 grid gap-2 sm:grid-cols-2">
            {OUTCOMES.map((o) => (
              <li key={o} className="flex items-start gap-2 text-sm text-charcoal/70">
                <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-brand" aria-hidden /> {o}
              </li>
            ))}
          </ul>
        </section>
                  ...
                  <ul className="divide-y divide-charcoal/5 rounded-xl border border-charcoal/10">
                    {m.lessons.map((l) => (
                      <li key={l.id} className="flex items-center gap-3 px-4 py-3 text-sm">
                        {l.isFreePreview ? <PlayCircle className="h-4 w-4 shrink-0 text-brand" aria-hidden /> : <Lock className="h-4 w-4 shrink-0 text-charcoal/30" aria-hidden />}
        ...
        <section aria-labelledby="instructor" className="reveal mt-8">
          <h2 id="instructor" className="font-heading text-xl font-bold">Your instructor</h2>
          <Card className="mt-3 flex items-center gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-brand/10 font-heading text-lg font-bold text-brand" aria-hidden>A</div>
            <div>
              {/* COPY: draft */}
        ...
        <section aria-labelledby="pricing" className="reveal mt-8">
          <h2 id="pricing" className="font-heading text-xl font-bold">How to get access</h2>
          <Card className="mt-3">
            {comingSoon ? (
              <>
                ...
                <div className="mt-4 max-w-xs"><Link href="/packages"><Button>See packages</Button></Link></div>
              </>
            ) : (
              <>
                <p className="text-sm text-charcoal/70">
                  ...
                  Included in{" "}
                  {inPackages.length > 0 ? inPackages.join(" & ") : "our packages"}.
                  {sb && cb && ` Skill Builder ${priceLabel(sb.priceInPaise)} (pick one course) or Career Booster ${priceLabel(cb.priceInPaise)} (both + future). GST-inclusive, no hidden charges.`}
                </p>
                <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                  <div className="sm:w-56"><Link href="/checkout?package=career-booster"><Button>Get started</Button></Link></div>
                  <div className="sm:w-48"><Link href="/packages"><Button variant="outline">Compare packages</Button></Link></div>
                </div>
              </>
            )}
          </Card>
