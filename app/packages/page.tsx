...
export default async function PackagesPage() {
  ...
  if (!sb || !cb) {
    return (
      <>
        <SiteHeader />
        <main className="mx-auto w-full max-w-md px-4 py-16 text-center text-charcoal/60">Packages are being set up. Please check back soon.</main>
        <SiteFooter />
      </>
    );
  }
    ...
    <>
      ...
      <main className="mx-auto w-full max-w-4xl px-4 pb-28 pt-10 md:pb-12">
        <header className="text-center">
          ...
          <p className="mx-auto mt-2 max-w-xl text-charcoal/60">One price, GST included. Koi hidden charge nahi — and a 48-hour refund if it&apos;s not for you.</p>
        </header>
            ...
            <table className="w-full min-w-[32rem] border-collapse text-sm">
              <thead>
                <tr className="border-b border-charcoal/10 text-left">
                  <th scope="col" className="py-3 pr-4 font-medium text-charcoal/50">Feature</th>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.feature} className="border-b border-charcoal/5">
                    <th scope="row" className="py-3 pr-4 text-left font-normal text-charcoal/60">{r.feature}</th>
