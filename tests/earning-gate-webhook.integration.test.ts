...
describe.skipIf(!HAS_DB)(
  "DR-038 earning gate in the webhook (B1, Tier-A)",
  () => {
    beforeAll(async () => {
    ...
    it("refund → clawback reverses the eligible commissions; wallets net to zero even after the hold", async () => {
      const { orderId, paymentId, byLevel } = await runScenario([
        true,
        true,
        true,
      ]);
    ...
    it("duplicate delivery is idempotent — the eligible credits are not doubled", async () => {
      const { orderId, razorpayOrderId, paymentId } = await runScenario([
        true,
        false,
        true,
      ]);
