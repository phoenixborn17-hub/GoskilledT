...
import { verifyPaymentSignature, verifyWebhookSignature } from "../modules/payments/razorpay";
import { decideWebhookActions, type OrderState } from "../modules/payments/webhook-flow";
...
import { coursesToEnroll, autoEnrollOnPublish } from "../modules/lms/entitlement";
import { validateWithdrawal, MIN_WITHDRAWAL_PAISE, MAX_WITHDRAWAL_PAISE } from "../modules/wallet/withdrawal";
...
describe("GST math (DR-023 inclusive pricing)", () => {
  ...
  it("rate 0 (D-03 not registered) → no GST", () => {
    expect(gstFromInclusive(149900, 0)).toEqual({ totalInPaise: 149900, baseInPaise: 149900, gstInPaise: 0 });
  });
...
describe("Razorpay signatures (Golden Rule 2)", () => {
  ...
  it("valid payment signature passes; tampered fails", () => {
    ...
    expect(verifyPaymentSignature({ razorpayOrderId: "order_1", razorpayPaymentId: "pay_1", signature: sig }, secret)).toBe(true);
...
describe("Webhook decision flow", () => {
  const order: OrderState = { id: "o1", status: "CREATED", amountInPaise: 149900 };
  const paid: OrderState = { ...order, status: "PAID", paidAt };
  it("golden path: capture → MARK_PAID + ENROLL + CREDIT", () => {
    const d = decideWebhookActions({ kind: "payment.captured", orderId: "o1", paymentId: "p1", amountInPaise: 149900 }, order, { alreadyProcessed: false });
    expect(d.actions.map((a) => a.do)).toEqual(["MARK_PAID", "ENROLL", "CREDIT_COMMISSIONS"]);
  });
  it("duplicate webhook → no actions (idempotent)", () => {
    expect(decideWebhookActions({ kind: "payment.captured", orderId: "o1", paymentId: "p1", amountInPaise: 149900 }, order, { alreadyProcessed: true }).actions).toEqual([]);
  });
  it("already-paid order → skip (no double credit)", () => {
    expect(decideWebhookActions({ kind: "payment.captured", orderId: "o1", paymentId: "p1", amountInPaise: 149900 }, paid, { alreadyProcessed: false }).actions).toEqual([]);
  });
  it("amount mismatch → manual review, NO credit", () => {
    const d = decideWebhookActions({ kind: "payment.captured", orderId: "o1", paymentId: "p1", amountInPaise: 99900 }, order, { alreadyProcessed: false });
  ...
  it("refund within 48h → refund + revoke + CLAWBACK (DR-025)", () => {
    const d = decideWebhookActions({ kind: "refund.processed", orderId: "o1", refundId: "r1", amountInPaise: 149900 }, paid, { alreadyProcessed: false, now: inWin });
    expect(d.actions.map((a) => a.do)).toEqual(["MARK_REFUNDED", "REVOKE_ENROLLMENTS", "CLAWBACK_COMMISSIONS"]);
  });
  it("refund after 48h → manual review, no auto-clawback (DR-025)", () => {
    const d = decideWebhookActions({ kind: "refund.processed", orderId: "o1", refundId: "r1", amountInPaise: 149900 }, paid, { alreadyProcessed: false, now: afterWin });
    expect(d.actions.map((a) => a.do)).toEqual(["MARK_REFUNDED", "REVOKE_ENROLLMENTS", "FLAG_MANUAL_REVIEW"]);
  });
});
...
describe("Upline resolution (DR-007)", () => {
  it("3 levels max, in order", () => {
    expect(resolveUplines("buyer", ["a", "b", "c", "d"])).toEqual([
      { userId: "a", level: 1 }, { userId: "b", level: 2 }, { userId: "c", level: 3 },
    ]);
  });
  it("stops on self-referral and cycles", () => {
    expect(resolveUplines("buyer", ["buyer", "a"])).toEqual([]);
    expect(resolveUplines("buyer", ["a", "b", "a"])).toEqual([{ userId: "a", level: 1 }, { userId: "b", level: 2 }]);
  });
...
describe("Commission credit builder (DR-007 + DR-025 hold)", () => {
  const txns = buildCommissionTxns({ orderId: "o1", pkg: "career-booster", paidAt, uplines: resolveUplines("buyer", ["a", "b", "c"]) });
  it("3 balanced txns with correct amounts ₹1250/250/150", () => {
    expect(txns).toHaveLength(3);
    expect(txns.map((t) => t.legs[1].amountInPaise)).toEqual([125000, 25000, 15000]);
    for (const t of txns) expect(t.legs.reduce((s, l) => s + l.amountInPaise, 0)).toBe(0);
  });
  it("wallet legs HELD until paidAt+48h; payable legs unheld", () => {
    for (const t of txns) {
      expect(t.legs[1].holdUntil?.toISOString()).toBe(commissionHoldUntil(paidAt).toISOString());
...
describe("Clawback builder (DR-025)", () => {
  const credits = buildCommissionTxns({ orderId: "o1", pkg: "skill-builder", paidAt, uplines: resolveUplines("buyer", ["a", "b"]) });
...
describe("Entitlement (DR-021)", () => {
  const sb = { slug: "skill-builder" as const, includesFutureCourses: false, courseIds: ["ai", "dm"] };
  const cb = { slug: "career-booster" as const, includesFutureCourses: true, courseIds: ["ai", "dm"] };
...
describe("Withdrawal rules (D-01 gate + bounds + held-exclusion)", () => {
  const base = { payoutsEnabled: true, kycStatus: "APPROVED" as const, hasPendingWithdrawal: false, availableInPaise: 100000, amountInPaise: 60000 };
  it("happy path ok", () => expect(validateWithdrawal(base)).toEqual({ ok: true }));
  ...
  it("KYC, single-pending, bounds, insufficient", () => {
    expect((validateWithdrawal({ ...base, kycStatus: "SUBMITTED" }) as never)["code"]).toBe("KYC_REQUIRED");
    expect((validateWithdrawal({ ...base, hasPendingWithdrawal: true }) as never)["code"]).toBe("PENDING_EXISTS");
    expect((validateWithdrawal({ ...base, amountInPaise: MIN_WITHDRAWAL_PAISE - 1 }) as never)["code"]).toBe("BELOW_MIN");
    expect((validateWithdrawal({ ...base, amountInPaise: MAX_WITHDRAWAL_PAISE + 1 }) as never)["code"]).toBe("ABOVE_MAX");
    expect((validateWithdrawal({ ...base, amountInPaise: 100001 }) as never)["code"]).toBe("INSUFFICIENT");
  });
});
...
describe("Wallet summary (DR-025 UX: held visible, locked)", () => {
  it("total/held/available/lifetime", () => {
    ...
    expect(s).toEqual({ totalInPaise: 110000, heldInPaise: 90000, availableInPaise: 20000, lifetimeEarnedInPaise: 130000 });
  });
});
